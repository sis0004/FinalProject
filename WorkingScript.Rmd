...
LabData <- as.data.frame(read.csv("/Users/saraschredergomes/Desktop/GEOG693/FinalProject/BrineCompostions_ActLabsResults_RAWData.csv", stringsAsFactors = FALSE), header= FALSE, sep= ",", dec = ".", col.names = NULL) # stringsasFactors doesnt seem to be working
...
for (col in colnames(LabData)){
    ...
    #remove values that are less than multiplied value
}
```
...
```{r} 
# if cell contains '<', replace with NA 
# https://stackoverflow.com/questions/42978715/if-string-exists-replace-cell-with-na


for (col in colnames(REELabData)){
    is.na(REELabData[[col]]) <- startsWith(REELabData[[col]], "<")
}
