...
# need a fix to include samples 9-11? no; only interested in acid saline lakes
TestMerge <- merge(FieldData, MajorIonLabData, by.x = "Sample", by.y = "Sample")
```
...
I want to see how it looks if you have Fe, Al, and Si vs. pH where they get different colors

----Get rid of NA value in plot
----add labels, make scales look nicer
        -------something is funky with y-axis 

Plots: pH v. Al 
        pH v. Si 
        etc.
        Maybe, for each lake plot pH v. series of rare earth elements to see if there is one that stands out?
### USE THIS? Using base R: Dot plots? http://www.sthda.com/english/wiki/dot-charts-r-base-graphs

```{r}
# Create basic scatter plot
ggplot(TestMerge, aes(x=pH, y=Fe)) + geom_point()

# Change the point size, and shape
ggplot(TestMerge, aes(x=pH, y=Fe)) +
  geom_point(size=3, shape=20) +
  geom_text(label=TestMerge$Location)

ggplot(TestMerge, aes(x=pH, y=Al)) +
  geom_point(size=3, shape=20) +
  geom_text(label=TestMerge$Location)

ggplot(TestMerge, aes(x=pH, y=Si)) +
  geom_point(size=3, shape=20) +
  geom_text(label=TestMerge$Location)
