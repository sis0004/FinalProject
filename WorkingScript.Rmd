...
### pH vs. Al scatter plot using ggplot2


### Loop / funciton? that creates scatter plots for any element input

```{r}
# Create basic scatter plot

# get user input of elements to use
# loop through columns entered OR all columns in data frame searching for matches
# create plot for matched column

# for (col in EnteredList)
#{
#   find matching column: data <- TestMerge$col
#   plot : ggplot(TestMerge, aes(x=as.numeric(pH), y=as.numeric(col))) + xlab("pH") + ylab(col) + geom_point() + geom_text(aes(label=Sample), vjust=-.5)  
#}
...
ggplot(MergedDatasets, aes(x=as.numeric(pH), y=as.numeric(Fe))) + xlab("pH") + ylab("Fe (mg/L)") + geom_point(size = 3, shape = 20) + geom_text(aes(label=Sample), vjust=-.5)  

ph <- MergedDatasets$pH
fe <- MergedDatasets$Fe
plot(ph, fe, main="Generic Plot", xlab="Ph", ylab="Fe", pch=19, panel.first=grid())


```

```{r}
ggplot(MergedDatasets, aes(x=pH, y=Y))+
  geom_point(size=3, shape=20) +
  geom_text(label=MergedDatasets$Location)


```


Line plot of rare earth elements
Goal: see if there are any correlations between pH and the occurance of rare earth elements

```{r}
library(plotly)

MergedDatasets <- MergedDatasets[order(MergedDatasets$pH),] # sort pH in ascending order

#plot selcted rare earth elements vs. pH as line plot
